...
#include <sys/un.h>
#include <sys/stat.h>
#include "kxmsglocalserver_unix.h"
kxMsgLocalServer::kxMsgLocalServer(QObject *parent) : m_server(NULL)
{
}

kxMsgLocalServer::~kxMsgLocalServer()
{
	stopServer();
}

bool kxMsgLocalServer::makeDir(const char* path, mode_t mode)
{
...
bool kxMsgLocalServer::startServer(const QString &strPortPath)
{
	if (isStarted())
		return false;
...
void kxMsgLocalServer::stopServer()
{
	if (isStarted())
	{
		m_server->close();
...
bool kxMsgLocalServer::isStarted()
{
	return m_server != NULL && m_server->isListening();
}

int kxMsgLocalServer::sendRequest(QLocalSocket *client, const QByteArray& byteData)
{
...
void kxMsgLocalServer::onNewConnection()
{
...
void kxMsgLocalServer::onReadyRead()
{
...
void kxMsgLocalServer::doParserRequest(QLocalSocket *socket, const QByteArray& byte)
{
//与Coding/shell2/exe/kxmsglocalclient_unix.h一致
#pragma pack(1)
	struct CmdMsgData
	{
...
void kxMsgLocalServer::onDisconnected()
{
	QLocalSocket *socket = qobject_cast<QLocalSocket *>(sender());
	socket->close();
	socket->deleteLater();

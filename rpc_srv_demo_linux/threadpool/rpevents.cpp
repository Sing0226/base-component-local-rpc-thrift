...
#include <deque>
    ...
    typedef smart_wfmo_t_ *smart_wfmo_t;
    ...
    struct smart_wfmo_info_t_ {
        smart_wfmo_t Waiter;
        int WaitIndex;
    };
    typedef smart_wfmo_info_t_ *smart_wfmo_info_t;
#endif // WFMO
...
#ifdef WFMO
        std::deque<smart_wfmo_info_t_> RegisteredWaits;
#endif
    };

#ifdef WFMO
    static bool RemoveExpiredWaitHelper(smart_wfmo_info_t_ wait) {
    ...
    smart_event_t CreateEvent(bool manualReset, bool initialState) {
        smart_event_t event = new smart_event_t_;
    ...
    static int UnlockedWaitForEvent(smart_event_t event, uint64_t milliseconds) {
        int result = 0;
    ...
    int WaitForEvent(smart_event_t event, uint64_t milliseconds) {
    ...
    int WaitForMultipleEvents(smart_event_t *events, int count, bool waitAll,
        ...
        smart_wfmo_t wfmo = new smart_wfmo_t_;
    ...
    int SetEvent(smart_event_t event) {
        ...
        if (event->AutoReset) {
            ...
            while (!event->RegisteredWaits.empty()) {
                smart_wfmo_info_t i = &event->RegisteredWaits.front();
            ...
            for (size_t i = 0; i < event->RegisteredWaits.size(); ++i) {
                smart_wfmo_info_t info = &event->RegisteredWaits[i];
